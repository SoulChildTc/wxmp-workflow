...
- 处理用户已有内容
- 撰写正文（标题、开头、段落、语言、结构、结尾、字数、图片、时间意识）
- 排版规范与英文处理
- 来源标注与外链限制
- 多轮修改
...
写文章时必须基于当前的真实日期。下笔前确认今天的日期，所有时间表述都要准确：

- **事件时间线要准。** 如果某件事是 3 天前发生的，就说"3 天前"或"上周"，而不是模糊的"最近"。如果不知道具体日期，用 `date` 命令获取当前时间。
- **预判选题注意时态。** 如果文章是在事件发生前写的（比如预判选题模式），用"即将""将在"等将来时；如果事件已经发生，用"已经""刚刚"等过去时。选题阶段记录的事件日期可以帮助判断。
- **避免 AI 式模糊时间词。** "近年来""最近一段时间""前不久"——都用具体时间替换。写不出来具体时间的，说明这个素材不够可靠，不应该用在文章里。
- **时效性标注。** 如果文章提到的数据或信息有明确时效，在文末加一句说明，比如"本文数据截至 2026 年 6 月"。
