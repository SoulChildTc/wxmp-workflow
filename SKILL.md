...
**量化评分（4 轮扫描后执行）：** 按特征词密度（25%）、句子长度方差（20%）、段落长度方差（10%）、具体性密度（15%）、口语化表达（10%）、数据密度（10%）、结构工整度（10%）加权评分，1-3 分自然通过，4+ 分进入迭代修复循环，直到评分 ≤ 4 分或连续 2 轮边际收益 < 0.5 分。

**独立评估：** 4 轮扫描和量化评分 MUST 由独立的子 agent 执行，不能和写稿的 agent 是同一个。避免"自己改自己评"的偏差。
