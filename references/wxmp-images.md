...
AI 生成配图前，先为这篇文章创造一套专属视觉风格。每篇文章风格不同——不固定、不重复、不从库里抽。
...
3. **AI 生成** — 用 wxmp-style-generator.md 的流程先创造风格，再用风格 + 图片内容构造 prompt，调用 Agnes AI 或 SenseNova 生成。优先 Agnes，Agnes 失败时切换 SenseNova
