...
**MUST 在任何工作流步骤之前执行，不可跳过。** 读取 `config/wxmp.json` 中的 `config_schema_version` 字段，与当前版本比对（当前版本见 `references/wxmp-setup.md` 的「配置版本历史」）。

- 版本匹配 → 继续，输出状态标记
- 字段不存在或低于当前版本 → 提示用户：「检测到配置需要更新，新增了 SenseNova U1.5 Lite 模型支持等配置项。建议输入"帮我配置"更新。」
  - 用户选择更新 → 引导到配置助手（`references/wxmp-setup.md`），完成后继续工作流
  - 用户选择跳过 → 继续工作流，新功能使用默认值，不再重复提示
...
> ⚠️ **关键限制：** 主 Agent 不得自行执行「4 轮扫描 + 量化评分」的独立评估。
> 必须调用子 Agent（task tool, subagent_type: general）执行。这是硬性要求，不是建议。
...
1. 查特征词 + 句型指纹：替换 AI 高频词（赋能/深耕/打造）、删除套话（值得注意的是/在当今社会）、检测 6 种 AI 句式模式（否定平行结构、虚假范围、三段式排比、模糊归因、僵化结尾、说教式开头）
2. 查结构 + 句子节奏：打破刻板结构、排比堆砌、段落雷同，检测句子长度标准差（<5 需修复），检测是否使用"第一/第二/第三"分层结构
3. 查风格：允许平淡——用具体细节和观察代替情绪词，不要强行煽情。检查数据是否过于密集（每个观点都配数据需删减）
4. 加人味：注意人味不是情绪堆砌——用具体细节和自然节奏制造人味，比用"太牛了""太离谱了"更高级
...
1. 调用子 agent
2. 子 Agent 执行：第 1 轮特征词扫描 → 第 2 轮结构节奏 → 第 3 轮风格 → 第 4 轮人味 → 量化评分
3. 子 Agent 输出评分报告（1-10 分）
4. 评分 ≤ 2 分 → 通过，进入下一步
5. 评分 > 2 分 → 主 Agent 修复后重新提交子 Agent 评估
