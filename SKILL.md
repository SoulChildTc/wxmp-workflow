...
- 版本匹配 → 继续，输出 `✅ 步骤 0：预检 — 配置版本已确认`
- 字段不存在或低于当前版本 → MUST 等用户选择，不可自动跳过：
  - 提示：「检测到配置需要更新，输入"帮我配置"更新，或输入"跳过"继续。」
  - 用户选更新 → 引导到配置助手（`references/wxmp-setup.md`），完成后输出 `✅ 步骤 0：预检 — 已更新配置`
  - 用户选跳过 → 输出 `✅ 步骤 0：预检 — 用户选择跳过版本更新`，继续工作流，不再重复提示
